...
import { PULSE_RING_FRAGMENT_SHADER, PULSE_RING_VERTEX_SHADER } from "./webgl2PulseRingShader";
...
export interface WebglPulseRingRenderOptions {
  readonly reducedMotion?: boolean;
  readonly timeSeconds?: number;
}

export interface WebglPulseRingRenderer {
  ...
  render(
    ...
    config?: PulseRingConfigInput | PulseRingConfig,
    options?: WebglPulseRingRenderOptions,
  ): boolean;
  resize(
    cssWidth: number,
    cssHeight: number,
...
export interface WebglPulseRingRendererOptions {
...
export function createWebglPulseRingRenderer(
  ...
  return new PulseRingRenderer(canvas, options);
}

class PulseRingRenderer implements WebglPulseRingRenderer {
  readonly capabilities = WEBGL2_RENDERER_CAPABILITIES;
  readonly id = "webgl2" as const;
  private readonly bandBuffer = new Float32Array(16);
  private readonly canvas: HTMLCanvasElement;
  private readonly listeners = new Set<() => void>();
  private readonly options: WebglPulseRingRendererOptions;
  private destroyed = false;
  private gl: WebGL2RenderingContext | null = null;
  private resources: PulseRingResources | null = null;
  private status: WebglRendererStatus = status("initializing", "Initializing WebGL2.", false, 0);
  private diagnostics = mutableDiagnostics();

  constructor(canvas: HTMLCanvasElement, options: WebglPulseRingRendererOptions) {
    this.canvas = canvas;
  ...
  render(
    ...
    const values = createPulseRingUniformState(frame, input);
    this.bandBuffer.fill(0);
    values.bands.forEach((energy, index) => (this.bandBuffer[index] = energy));
    ...
    gl.uniform1f(
      uniforms.u_time,
      resolvePulseRingTime(options.timeSeconds ?? 0, options.reducedMotion === true),
    );
    ...
    setColor(gl, uniforms.u_background_color, values.backgroundColor);
    setColor(gl, uniforms.u_primary_color, values.primaryColor);
    setColor(gl, uniforms.u_secondary_color, values.secondaryColor);
    setColor(gl, uniforms.u_tertiary_color, values.tertiaryColor);
    setColor(gl, uniforms.u_sweep_color, values.sweepColor);
  ...
  private rebuildResources() {
    ...
    this.releaseResources();
    this.resources = createResources(gl, this.options, this.diagnostics);
    this.diagnostics.generation += 1;
    this.setStatus(status("ready", "WebGL2 Pulse Ring ready.", true, this.diagnostics.generation));
  }
...
function createResources(
  ...
  const vertexShader = compileShader(
    gl,
    gl.VERTEX_SHADER,
    options.vertexShaderSource ?? PULSE_RING_VERTEX_SHADER,
  );
  ...
  try {
    fragmentShader = compileShader(
      gl,
      gl.FRAGMENT_SHADER,
      options.fragmentShaderSource ?? PULSE_RING_FRAGMENT_SHADER,
    );
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS))
      throw new WebglRendererError(
        "WEBGL2_PROGRAM_LINK",
        `Pulse Ring program link failed: ${gl.getProgramInfoLog(program) || "No linker log."}`,
      );
    ...
    const uniforms = Object.fromEntries(
      UNIFORM_NAMES.map((name) => {
        ...
        if (!location)
          throw new WebglRendererError(
            "WEBGL2_UNIFORM_MISSING",
            `Pulse Ring shader is missing required uniform ${name}.`,
          );
...
function compileShader(gl: WebGL2RenderingContext, kind: number, source: string): WebGLShader {
  ...
  throw new WebglRendererError(
    "WEBGL2_SHADER_COMPILE",
    `Pulse Ring shader compilation failed: ${log}`,
  );
}
