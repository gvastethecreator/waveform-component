...
- `BUILTIN_RENDERER_CATALOG` extends that capability query with WebGL2 VFX surfaces without widening core visualization configs. WebGL2 accepts `pulse-ring`, `neon-lines`, or `equalizer-grid` plus `BandEnergyFrame`, reports a 16-band ceiling, and never silently impersonates Canvas/SVG/DOM.
- `createWebglBandVfxRenderer` is the shared lifecycle boundary behind the three public effect adapters. It owns exactly one context listener set and one live program/buffer/vertex-array tuple. Context loss is cancelable and invalidates the tuple; restoration publishes `restoring`, recompiles and relinks the selected clean-room shader, rebinds every common/effect uniform, and increments a generation. Unavailable/compile/link/restore failures remain visible statuses. Disposal is idempotent and textures remain at zero.
- VFX config is separated from uniform derivation. Shared helpers clamp/reduce hostile band input to 16 values and compute energy, peak, and centroid once. Effect resolvers bound line count to 12 and grid coordinates to 48×24 before shader work. Typed control definitions publish complete parameter semantics, while immutable presets hold fully resolved configs.
- `resolveWebglDrawingBufferSize` isolates responsive CSS sizing from bounded GPU allocation. Quality caps DPR before the 4,096-dimension and 4,194,304-pixel ceilings. `BandVfxSurface` owns one observer and, only in full motion, one requestAnimationFrame chain; reduced motion performs one deterministic draw. Context loss or adapter change cancels that chain and effect-specific React CSS fallbacks keep the surface meaningful.
