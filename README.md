...
export function AudioRing({ spectrum }: { spectrum: SpectrumFrame }) {
  ...
  return (
    <>
      ...
      <WaveformRibbon
        data={bands}
        config={{ waveHeight: 0.22, reflectionStrength: 0.38, energyReactivity: 1.15 }}
      />
      <RoundedWobbleBars
        data={bands}
        config={{ barCount: 28, mirrorVertically: true, wobbleIntensity: 0.34 }}
      />
      <SpectrumBars
        data={bands}
